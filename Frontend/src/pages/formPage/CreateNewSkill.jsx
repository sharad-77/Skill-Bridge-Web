...
const skillFormSchema = z.object({
  ...
  highlights: z.array(z.string()).min(5, 'At least 5 highlights are required'),
  knowledgeRequirement: z.array(z.string()).min(5, 'At least 5 knowledge requirements are required'),
});
...
const CreateNewSkill = ({ onClose = () => { } }) => {
  ...
  const nextStep = async () => {
    ...
    if (currentStep === 3) valid = await trigger(['highlights', 'knowledgeRequirement']);
  ...
  const onSubmit = async (data) => {
    // Provide dummy values if empty
    const payload = {
      ...data,
      image: data.image || 'https://dummyimage.com/600x400/cccccc/000000&text=No+Image',
      video: data.video || 'https://www.example.com/dummy-video.mp4',
    };
    try {
